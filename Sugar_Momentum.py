...
# Bangun model dengan GD + momentum
def build_model():
    model = Sequential()
...
model = build_model()
model.fit(X_train, y_train, epochs=100, batch_size=len(X_train), verbose=0)  # GD full batch
...
plt.title('Total Sugar Consumption Indonesia - Model Neural Network (GD + Momentum)')
plt.xlabel('Tahun')
